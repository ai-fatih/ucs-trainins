...
| Центральный guard редиректов (матрица роутов) | `components/layout/AuthRouter.tsx` + `lib/auth/route-access.ts` | localStorage `ucs-auth` | ✅ защищает `/booking`, `/bookings`, `/chat`, `/profile`, `/settings`, `/notifications`, `/review`, `/school/*`, `/dashboard`; лендинг и `/auth/*` — guest-only |
...
| Визард записи (4 шага) | `app/booking/page.tsx` | `stores/booking.ts`, `data/slots.json` | ✅ выбор специалиста, динамический календарь |
| Мои записи | `app/bookings/page.tsx` | `data/bookings.json` | ✅ |
| Отмена / перенос / история | `app/bookings/page.tsx` | mock | ✅ отмена (подтверждение) + перенос (модалка с календарём) |
| Защита от double-booking | — | — | ⏳ |
| Чаевые (TipModal) | `components/features/TipModal.tsx` | `lib/tips/storage.ts` (localStorage) | ✅ мок-квитанция (оплата НетМонет/СберЧаевые — backend) |
| Опрос качества | `components/features/QualitySurveyModal.tsx` | `lib/feedback/storage.ts` (localStorage) | ✅ сохранение ответов |
...
| Фича | Файл | Данные | Статус |
|------|------|--------|--------|
| Список чатов | `app/chat/page.tsx` | `data/chats.json` | ✅ (вход: header-таб «Чат») |
| Чат с менеджером (сообщения, файлы) | `app/chat/[id]/page.tsx` | mock | 🔧 мок → backend |
...
| Фича | Файл | Данные | Статус |
|------|------|--------|--------|
| Отзыв после консультации | `app/review/page.tsx` | mock (специалист из записи `?bookingId=`) | ✅ |
...
| Рейтинг | `app/school/leaderboard/page.tsx` | mock | ✅ |
...
| Профиль школы (редактирование) | `app/school/profile/page.tsx` | `lib/school/storage.ts` (localStorage) | ✅ |
...
| Фича | Файл | Данные | Статус |
|------|------|--------|--------|
| Профиль (данные, сотрудники компании, экспорт/удаление) | `app/profile/page.tsx` | mock | ✅ |
| Настройки (заглушка) | `app/settings/page.tsx` | — | ⚠️ разделы «скоро» |
| Настройки уведомлений (каналы/события) | `app/notifications/page.tsx` | `data/notification-channels.json`, `data/notification-events.json` | ✅ (обратная ссылка на профиль) |
