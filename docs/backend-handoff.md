...
> **Актуализация 03.08.2026:** текущий контур — публичный информационно-обучающий портал без ЛК (см. `docs/brief-public-phase.md`). Backend свёрнут до `GET /health`; заявки, feedback, consent, авторизация, админка, бронирования, чат, отзывы и уведомления **удалены** (реализация в ветке `main-arhive`). Ниже — исторические контракты на будущее восстановление.
