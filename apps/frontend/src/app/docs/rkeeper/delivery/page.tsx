...
const delivery = docProducts.find((p) => p.id === 'delivery')!;
...
const sections = [
  {
    ...
    items: delivery.scenarios
      .filter((s) => s.href.includes('create-order') || s.href.includes('call-center'))
      .map(toItem),
  },
  {
    title: 'Мобильные приложения',
    items: delivery.scenarios
      .filter((s) => s.href.includes('courier-app'))
      .map(toItem),
  },
];
